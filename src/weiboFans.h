class WeiboFansApplication : public Application {
private:
    uint32_t numColor = 0xFFFFFF;
    uint32_t backgroundColor = 0x000000;
    long weiboFansCount = 0;
    bool selected = false;
    static const uint32_t logo[8][8];
public:
    WeiboFansApplication() : Application("weiboFans", "微博粉丝计数器", {
            ConfigItem("uid", "微博UID", "7826543514", "请输入微博UID", "^[1-9][0-9]{0,15}$", true, true),
    ...
    ScreenController::ScreenFrame getOutsideScreenFrame() override {
        ...
        for (uint8_t y = 0; y < 8; y++) {
            for (uint8_t x = 0; x < 8; x++) {
                frame.data[y][x] = logo[y][x];
            }
        }
        String weiboFansCountStr = String(weiboFansCount);
        uint8_t weiboFansCountStrLength = weiboFansCountStr.length();
        if (weiboFansCountStrLength <= 6) {
            uint8_t startX = 21 - weiboFansCountStrLength * 2;
            for (auto i: weiboFansCountStr) {
                uint8_t num = i - '0';
        ...
        } else if (weiboFansCountStrLength == 7) {
            uint8_t startX = 9;
            for (uint8_t i = 0; i < 4; i++) {
                uint8_t num = weiboFansCountStr[i] - '0';
                for (uint8_t y = 0; y < 5; y++) {
                    for (uint8_t x = 0; x < 3; x++) {
                        frame.data[y + 2][startX + x] = screen_nums[num][y][x] ? numColor : backgroundColor;
                    }
        ...
        } else if (weiboFansCountStrLength == 8) {
            uint8_t startX = 10;
            for (uint8_t i = 0; i < 4; i++) {
                uint8_t num = weiboFansCountStr[i] - '0';
                for (uint8_t y = 0; y < 5; y++) {
    ...
    [[noreturn]] void mainTask() override {
        ...
        while (true) {
            if (selected and WiFiClass::getMode() == WIFI_STA and WiFiClass::status() == WL_CONNECTED and xSemaphoreTake(ApplicationController::http_xMutex, (TickType_t) 1000) == pdTRUE) {
                HTTPClient http;
                http.begin("https://m.weibo.cn/api/container/getIndex?type=uid&value=" + config["uid"]);
                Serial.println("正在请求微博API...");
                int httpCode = http.GET();
                if (httpCode == 200) {
                    String payload = http.getString();
                    JsonDocument doc;
                    deserializeJson(doc, payload);
                    weiboFansCount = doc["data"]["userInfo"]["followers_count"].as<long>();
...
constexpr const uint32_t WeiboFansApplication::logo[8][8]{
        {0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000},
        {0x000000, 0xFF0000, 0x000000, 0xFF0000, 0xFF0000, 0x000000, 0x000000, 0x000000},
        {0x000000, 0xFF0000, 0xFF0000, 0xFF0000, 0xFF0000, 0xFF0000, 0xFF0000, 0x000000},
        {0x000000, 0xFF0000, 0xFFFFFF, 0xFFFFFF, 0xFFFFFF, 0xFF0000, 0x000000, 0x000000},
        {0x000000, 0xFF0000, 0xFFFFFF, 0x000000, 0xFFFFFF, 0xFF0000, 0xFF0000, 0x000000},
        {0x000000, 0xFF0000, 0xFFFFFF, 0xFFFFFF, 0xFFFFFF, 0xFF0000, 0xFF0000, 0x000000},
